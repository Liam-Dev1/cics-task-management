...
type ChartTab = "pieCharts" | "lineCharts" | "barCharts"
...
// Wrapper components to handle tooltip formatting
const AverageCompletionTimeChartWrapper = ({ data }: { data: any[] }) => {
  const formatTooltipValue = (value: any) => [`${value}%`, "Avg. Completion Time"]
  ...
  return (
    <AverageCompletionTimeChart
      data={data}
      // @ts-ignore - We know these props are used internally
...
const TaskCompletionStatusChartWrapper = ({ data }: { data: any[] }) => {
...
export default function TaskReportDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<ChartTab>("pieCharts")
  const [showGraphs, setShowGraphs] = useState(false)
  const [showReport, setShowReport] = useState(false)
  const [isExporting, setIsExporting] = useState(false)
  const [exportTimeFrame, setExportTimeFrame] = useState<TimeFrame>("monthly")
  const [exportTab, setExportTab] = useState<ChartTab>("pieCharts")

  // Initialize with default date range of past year to today
  ...
  // Cast to the required type to fix TypeScript error
  const reportRef = useRef<HTMLDivElement>(null) as React.RefObject<HTMLDivElement>

  // Load task data only once on component mount
  useEffect(() => {
  ...
  // Memoize all receivers to avoid recalculation
  const allReceivers = useMemo(() => {
    if (loading) return []
    return Array.from(new Set(taskData.tasks.map((task) => task.assignedTo)))
  }, [loading])

  const allTaskStatuses: TaskStatus[] = ["Completed", "Pending", "Overdue"]
  ...
  const filteredTasks = useMemo(() => {
    return taskData.tasks.filter((task) => {
      const matchesReceiver =
        appliedFilters.taskReceivers.length === 0 || appliedFilters.taskReceivers.includes(task.assignedTo)
      ...
      const isWithinDateRange = taskDate >= appliedFilters.fromDate && taskDate <= appliedFilters.toDate
  ...
  const stats = useMemo(() => {
    ...
    // Calculate averages only if there are completed tasks
    const completedTasksWithTime = filteredTasks.filter(
      (task) => task.status === "Completed" && task.completionTime !== null,
    )

    const avgCompletionTime = completedTasksWithTime.length
      ? Math.round(
          completedTasksWithTime.reduce((sum, task) => sum + (task.completionTime || 0), 0) /
            completedTasksWithTime.length,
        )
      : 0

    const avgCompletionRate = tasksAssigned ? Math.round((completedTasks / tasksAssigned) * 100) : 0
  ...
  const chartData = useMemo(() => {
    ...
    const completionTimingBreakdown = [
      {
        name: "Early (≤30%)",
        value: filteredTasks.filter((task) => task.completionTime != null && task.completionTime <= 30).length,
        color: "#8B2332",
      },
      {
        name: "Normal (31-60%)",
        value: filteredTasks.filter(
          (task) => task.completionTime != null && task.completionTime > 30 && task.completionTime <= 60,
        ).length,
        color: "#4A5568",
      },
      {
        ...
        value: filteredTasks.filter(
          (task) => task.completionTime != null && task.completionTime > 60 && task.completionTime <= 90,
        ).length,
        color: "#4A5568",
      },
      {
        name: "On Deadline (>90%)",
        value: filteredTasks.filter((task) => task.completionTime != null && task.completionTime > 90).length,
        color: "#1E1E1E",
      },
    ]
    ...
    const generateTimeBasedData = (
      timeFrame: TimeFrame,
      dataGenerator: (startDate: Date, endDate: Date, tasks: typeof filteredTasks) => any[],
    ) => {
      ...
      switch (timeFrame) {
        case "weekly":
          startDate = startOfWeek(fromDate)
      ...
      return dataGenerator(startDate, endDate, filteredTasks)
    }

    // Generate average completion time data
    const generateAverageCompletionTimeData = (startDate: Date, endDate: Date, tasks: typeof filteredTasks) => {
      const data = []
      const increment = timeFrame === "weekly" ? 7 : timeFrame === "monthly" ? 1 : timeFrame === "quarterly" ? 3 : 12

      const dateIncrement = (date: Date, inc: number) => {
      ...
      const formatPeriod = (date: Date) => {
        if (timeFrame === "weekly") {
          ...
          return `Week ${weekNumber} (${date.toLocaleString("default", { month: "short" })})`
      ...
      for (let date = new Date(startDate); date <= endDate; date = dateIncrement(date, increment)) {
        let periodEnd = dateIncrement(date, increment)
        ...
        const avgCompletionTime = tasksInPeriod.length
          ? Math.round(tasksInPeriod.reduce((sum, task) => sum + (task.completionTime || 0), 0) / tasksInPeriod.length)
          : 0

        data.push({
          name: formatPeriod(date),
    ...
    const generateTaskCompletionStatusData = (startDate: Date, endDate: Date, tasks: typeof filteredTasks) => {
      const data = []
      const increment = timeFrame === "weekly" ? 7 : timeFrame === "monthly" ? 1 : timeFrame === "quarterly" ? 3 : 12
    ...
    // Generate all time-based data for each timeframe
    const averageCompletionTimeData = {
      weekly: generateTimeBasedData("weekly", generateAverageCompletionTimeData),
      monthly: generateTimeBasedData("monthly", generateAverageCompletionTimeData),
      quarterly: generateTimeBasedData("quarterly", generateAverageCompletionTimeData),
      yearly: generateTimeBasedData("yearly", generateAverageCompletionTimeData),
    }

    const taskCompletionStatusData = {
      weekly: generateTimeBasedData("weekly", generateTaskCompletionStatusData),
      monthly: generateTimeBasedData("monthly", generateTaskCompletionStatusData),
      quarterly: generateTimeBasedData("quarterly", generateTaskCompletionStatusData),
      yearly: generateTimeBasedData("yearly", generateTaskCompletionStatusData),
    }
  ...
  }, [filteredTasks, stats, appliedFilters, timeFrame])

  // Event handlers as callbacks to prevent unnecessary re-renders
  ...
  }, [exportTimeFrame, exportTab])

  const handleExportTimeFrameChange = useCallback((newTimeFrame: string) => {
    setExportTimeFrame(newTimeFrame as TimeFrame)
  }, [])

  const handleExportTabChange = useCallback((newTab: string) => {
    setExportTab(newTab as ChartTab)
  }, [])
                            ...
                            <div className="mt-4 flex flex-col gap-2 lg:hidden">
                              ...
                              <ExportToPdfButton
                                ...
                                onTimeFrameChange={(timeFrame) => {
                                  setExportTimeFrame(timeFrame as TimeFrame)
                                  handleExportTimeFrameChange(timeFrame)
                                }}
                                onTabChange={(tab) => {
                                  setExportTab(tab as ChartTab)
                                  handleExportTabChange(tab)
                                }}
                              />
                            </div>
                          </div>
                      ...
                      <div className="mx-auto">
                        ...
                        {activeTab === "lineCharts" && (
                          <div className="max-w-[1600px] mx-auto" data-tab="lineCharts">
                            ...
                            <AverageCompletionTimeChartWrapper data={chartData.averageCompletionTimeData[timeFrame]} />
                          </div>
                        )}
                        ...
                        {activeTab === "barCharts" && (
                          <div className="max-w-[1600px] mx-auto" data-tab="barCharts">
                            ...
                            <TaskCompletionStatusChartWrapper data={chartData.taskCompletionStatusData[timeFrame]} />
                          </div>
                        )}
                      </div>
                    </TooltipProvider>
